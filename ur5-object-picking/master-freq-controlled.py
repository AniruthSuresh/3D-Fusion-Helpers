...
def update_simulation(
    ...
    dt = params["fixedTimeStep"]
    PHYSICS_HZ = int(1.0 / dt)

    CAPTURE_HZ = 30
    CAPTURE_EVERY = max(1, PHYSICS_HZ // CAPTURE_HZ)
    ...
    print(f"\n[SIM INFO]")
    print(f"  Physics timestep     : {dt:.6f} sec")
    print(f"  Physics frequency    : {PHYSICS_HZ} Hz")
    print(f"  Capture frequency    : {CAPTURE_HZ} Hz")
    print(f"  Capture every        : {CAPTURE_EVERY} steps")
    print(f"  Requested steps      : {steps}")
    print(f"  Expected sim time    : {steps * dt:.3f} sec")
    ...
    for step in range(steps):
        ...
        view_matrix_tp = p.computeViewMatrix(
            tp_cam_eye, tp_cam_target, tp_cam_up
        )

        _, _, rgb_tp, depth_tp, _ = p.getCameraImage(
            224, 224, view_matrix_tp, proj_matrix
        )

        rgb_tp = np.array(rgb_tp)[:, :, :3]
        depth_tp = np.array(depth_tp)
        pcd_tp = depth_to_point_cloud(depth_tp, view_matrix_tp, proj_matrix)
        ...
        # ============ WRIST CAMERA ============
        wr_pos, wr_target, wr_up = get_wrist_camera_params(robot)
        view_matrix_wr = p.computeViewMatrix(wr_pos, wr_target, wr_up)

        _, _, rgb_wr, depth_wr, _ = p.getCameraImage(
            224, 224, view_matrix_wr, proj_matrix
        )

        rgb_wr = np.array(rgb_wr)[:, :, :3]
        depth_wr = np.array(depth_wr)
        pcd_wr = depth_to_point_cloud(depth_wr, view_matrix_wr, proj_matrix)
        ...
        tp_extr = compute_extrinsics(
            np.array(tp_cam_eye) - base_pos_np,
            np.array(tp_cam_target) - base_pos_np,
            tp_cam_up,
        )

        wr_extr = compute_extrinsics(
            np.array(wr_pos) - base_pos_np,
            np.array(wr_target) - base_pos_np,
            wr_up,
        )
        save_camera_pose(
            {
                "frame": frame_counter[0],
                "third_person_camera": tp_extr,
                "wrist_camera": wr_extr,
            },
            dirs["poses"],
            frame_counter[0],
        )
    ...
    print(f"\n[TIMING SUMMARY]")
    print(f"  Simulated time       : {sim_time:.3f} sec")
    print(f"  Wall-clock time      : {wall_time:.3f} sec")
    print(f"  Frames captured      : {captured_frames}")
    print(f"  Effective FPS (wall) : {captured_frames / wall_time:.2f}")
    print(f"  Ideal capture FPS    : {CAPTURE_HZ}")
    print(f"  Speed factor         : {sim_time / wall_time:.2f}x\n")
...
def move_and_grab_cube(robot, tray_pos, base_save_dir="dataset"):
    ...
    while True:
        ...
        # Move above cube - 3 seconds at 30 fps = 90 frames
        # Physics runs at 240 Hz, so we need 240 * 3 = 720 physics steps
        # To capture 90 frames, we capture every 8 steps (720 / 90 = 8)
        robot.move_arm_ik([cube_start_pos[0], cube_start_pos[1], 0.83], eef_orientation)
        
        # Lower the max_velocity to ensure smooth motion over 3 seconds
        # Temporarily set a lower max velocity for smoother motion
        original_max_vel = robot.max_velocity
        robot.max_velocity = 0.1  # Slower movement for smoother interpolation
        
        # Re-issue the command with slower velocity
        robot.move_arm_ik([cube_start_pos[0], cube_start_pos[1], 0.83], eef_orientation)
        
        update_simulation(
            steps=720,  # 3 seconds at 240 Hz
            capture_frames=True,
        ...
        robot.max_velocity = original_max_vel
        
        # Remove cube
        p.removeBody(cube_id)

        print(f"Completed iteration {iteration} - {frame_counter[0]} frames captured")
        print(f"  Expected frames: 90")
        print(f"  Agent states shape: {len(state_history)}")
        iteration += 1
