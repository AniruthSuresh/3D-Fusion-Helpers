...
def create_data_folders(iter_folder):
    folders = {}
    for cam in ['third_person', 'wrist']:
        for dtype in ['rgb', 'depth', 'pcd_v1', 'pcd_v2', 'pcd_v3']:
            key = f'{cam}_{dtype}'
            path = os.path.join(iter_folder, cam, dtype)
            os.makedirs(path, exist_ok=True)
            folders[key] = path
    
    poses_dir = os.path.join(iter_folder, "camera_poses")
    os.makedirs(poses_dir, exist_ok=True)
    folders['poses'] = poses_dir
    
    return folders

def depth_to_point_cloud_v1(depth_buffer, view_matrix, proj_matrix, width=224, height=224):
    """Original version with Y flip only"""
    fov = 60
    near = 0.01
    far = 3.0
...
def depth_to_point_cloud_v2(depth_buffer, view_matrix, proj_matrix, width=224, height=224):
    ...
    # Standard OpenCV convention first
    z_cam = depth_img
    x_cam = (u - cx) * z_cam / fx
    y_cam = (v - cy) * z_cam / fy
    
    points_camera_opencv = np.stack([x_cam, y_cam, z_cam], axis=-1).reshape(-1, 3)
    
    # Convert OpenCV to OpenGL: [X, Y, Z]_opencv -> [X, -Y, -Z]_opengl
    points_camera_opengl = points_camera_opencv * np.array([1, -1, -1])
    
    points_camera_homogeneous = np.concatenate([points_camera_opengl, np.ones((points_camera_opengl.shape[0], 1))], axis=1)
    points_world = (view_matrix_inv @ points_camera_homogeneous.T).T[:, :3]
    
    return points_world

def depth_to_point_cloud_v3(depth_buffer, view_matrix, proj_matrix, width=224, height=224):
    """Direct OpenGL convention with proper signs"""
    fov = 60
    near = 0.01
    far = 3.0
    ...
    x_ndc = (u - width/2) / focal_length
    y_ndc = (v - height/2) / focal_length
    
    # OpenGL: camera looks down -Z, Y up
    x_cam = x_ndc * depth
    y_cam = -y_ndc * depth  # Flip Y
    z_cam = -depth  # -Z is forward
    
    points_cam = np.stack([x_cam, y_cam, z_cam], axis=-1).reshape(-1, 3)
    
    view_matrix_np = np.array(view_matrix).reshape(4, 4).T
    cam_to_world = np.linalg.inv(view_matrix_np)
    
    points_hom = np.concatenate([points_cam, np.ones((len(points_cam), 1))], axis=1)
    points_world = (cam_to_world @ points_hom.T).T[:, :3]
...
def update_simulation(steps, sleep_time=0.01, capture_frames=False, iter_folder=None, 
    ...
    for _ in range(steps):
        ...
        if capture_frames and iter_folder is not None and frame_counter is not None:
            ...
            pcd_tp_v1 = depth_to_point_cloud_v1(depth_buffer_tp, view_matrix_tp, proj_matrix)
            pcd_tp_v2 = depth_to_point_cloud_v2(depth_buffer_tp, view_matrix_tp, proj_matrix)
            pcd_tp_v3 = depth_to_point_cloud_v3(depth_buffer_tp, view_matrix_tp, proj_matrix)
            
            # Save third person
            cv2.imwrite(
                os.path.join(dirs['third_person_rgb'], f"tp_rgb_{frame_counter[0]:04d}.png"),
                cv2.cvtColor(rgb_tp, cv2.COLOR_RGB2BGR)
            )
            np.save(os.path.join(dirs['third_person_depth'], f"tp_depth_{frame_counter[0]:04d}.npy"), depth_buffer_tp)
            np.save(os.path.join(dirs['third_person_pcd_v1'], f"tp_pcd_v1_{frame_counter[0]:04d}.npy"), pcd_tp_v1)
            np.save(os.path.join(dirs['third_person_pcd_v2'], f"tp_pcd_v2_{frame_counter[0]:04d}.npy"), pcd_tp_v2)
            np.save(os.path.join(dirs['third_person_pcd_v3'], f"tp_pcd_v3_{frame_counter[0]:04d}.npy"), pcd_tp_v3)
            ...
            # Generate all 3 versions
            pcd_wr_v1 = depth_to_point_cloud_v1(depth_buffer_wr, view_matrix_wr, proj_matrix)
            pcd_wr_v2 = depth_to_point_cloud_v2(depth_buffer_wr, view_matrix_wr, proj_matrix)
            pcd_wr_v3 = depth_to_point_cloud_v3(depth_buffer_wr, view_matrix_wr, proj_matrix)
            
            # Save wrist
            cv2.imwrite(
                os.path.join(dirs['wrist_rgb'], f"wr_rgb_{frame_counter[0]:04d}.png"),
                cv2.cvtColor(rgb_wr, cv2.COLOR_RGB2BGR)
            )
            np.save(os.path.join(dirs['wrist_depth'], f"wr_depth_{frame_counter[0]:04d}.npy"), depth_buffer_wr)
            np.save(os.path.join(dirs['wrist_pcd_v1'], f"wr_pcd_v1_{frame_counter[0]:04d}.npy"), pcd_wr_v1)
            np.save(os.path.join(dirs['wrist_pcd_v2'], f"wr_pcd_v2_{frame_counter[0]:04d}.npy"), pcd_wr_v2)
            np.save(os.path.join(dirs['wrist_pcd_v3'], f"wr_pcd_v3_{frame_counter[0]:04d}.npy"), pcd_wr_v3)
...
def move_and_grab_cube(robot, tray_pos, base_save_dir="dataset"):
    iteration = 0
    while iteration < 1:  # Generate just 1 iteration for testing
        ...
        print(f"  Saved 3 versions of point clouds (v1, v2, v3)")
        iteration += 1
