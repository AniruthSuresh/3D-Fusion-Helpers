...
def process_traj(traj_name, db3_path):
    ...
    while reader.has_next():
        ...
        if topic == RGB_TOPIC and rgb_type:
            ...
            rgb_buf.append(msg)  # store raw msg for sync
            rgb_ts.append(ts)
    ...
    if not rgb_buf or not depth_buf or not joint_buf:
        print(f"Skipping {traj_name}: missing data")
        return
    ...
    # Synchronize all to RGB timestamps
    depth_idx = align_to(rgb_ts, depth_ts)
    joint_idx = align_to(rgb_ts, joint_ts)
    grip_idx = align_to(rgb_ts, grip_ts)

    joint_sync = np.array([joint_buf[i] for i in joint_idx])
    grip_sync = np.array([grip_buf[i] for i in grip_idx])

    # Build synchronized states: [joint1..n, gripper]
    states = np.hstack([joint_sync, grip_sync[:, None]]).astype(np.float32)

    # Build actions = diff(states)
    ...
    print(f"Saved {len(states)} synchronized states/actions for {traj_name}")
